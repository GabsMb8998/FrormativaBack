...
from app_smart.models import DadosSensores, TemperaturaData, UmidadeData, LuminosidadeData, ContadorData
...
class SensorFIlter(django_filters.FilterSet):
    ...
    tipo = django_filters.CharFilter(field_name='tipo__tipo', lookup_expr='exact')
...
class SensorFilterView(APIView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        filters = Q()
        if tipo:
            filters &= Q(tipo__tipo__icontains=tipo)
        if localizacao:
        ...
        queryset = DadosSensores.objects.filter(filters)
import django_filters
from app_smart.models import DadosSensores
from rest_framework import permissions
from app_smart.api import serializers
from rest_framework.response import Response
from rest_framework import status
...
class SensorFIlter(django_filters.FilterSet):
    ...
    status_operacional = django_filters.BooleanFilter(field_name='status_operacional', lookup_expr='exact')
